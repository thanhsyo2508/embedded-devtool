...
/// by tungstenite internally and never forwarded.
fn forward_message(msg: Message, tx: &Sender<Vec<u8>>) -> Option<bool> {
    match msg {
        Message::Text(text) => {
        ...
        Message::Binary(data) => Some(tx.send(data).is_ok()),
    ...
    fn open(&mut self) -> io::Result<()> {
        ...
        self.reader_thread = Some(thread::spawn(move || {
            while !stop_flag.load(Ordering::Relaxed) {
                ...
                match result {
                    Ok(msg) => match forward_message(msg, &tx) {
                        Some(true) => continue,
...
mod tests {
    ...
    fn ws_client_and_server_round_trip() {
        let port = 18281;
        let mut server = WsServerStream::new(port);
        server.open().unwrap();

        let mut client = WsClientStream::new(format!("ws://127.0.0.1:{port}/"));
