...
> **Status:** v0.1.4 — the full roadmap's core feature set is implemented and working end to end: serial monitor, SSH terminal, ESP32 + STM32 flashing (including WiFi OTA and batch/provisioning modes), an FTP client/server, the realtime plotter (with FFT and math channels), TCP/UDP/WebSocket/MQTT/SSH network tools (each with a protocol-appropriate terminal, not a generic byte stream), a LAN network scanner, RS485/Modbus RTU + Modbus TCP, a Lua scripting engine plus an installable Lua plugin system (custom decoders/plotter parsers), an optional local REST API for CI/production-line integration, a command palette (Ctrl+K), a VS Code-style multi-pane split layout, English/Vietnamese UI language, and auto-update via GitHub Releases. See [docs/quickstart.md](docs/quickstart.md) to build and try it, and [CHANGELOG.md](CHANGELOG.md) for what shipped in this release. See [docs/ke-hoach-phat-trien-embedded-devtool.md](docs/ke-hoach-phat-trien-embedded-devtool.md) for the full roadmap and [docs/ke-hoach-chi-tiet-giai-doan-0-1.md](docs/ke-hoach-chi-tiet-giai-doan-0-1.md) for the detailed task breakdown.
...
- Dark/light/system theme, global keyboard shortcuts, persisted settings, an in-app User Guide with worked examples for every feature, English/Vietnamese UI language (switchable in Settings, no restart needed). A toast notification reports success/failure for batch flash, auto-flash-on-plug, provisioning, OTA, and FTP transfers regardless of which panel or tab is currently open — a notification-bell dropdown in the top bar keeps the last 50 so a missed one isn't gone for good. The connect panel remembers your last few connections (across every protocol) for one-click reconnect, on top of named saved profiles.

Not yet built: WASM plugins (current plugins are Lua-only), installing a plugin from a URL (file only for now), and custom panels contributed by a plugin — see the roadmap docs above for later-phase items.
